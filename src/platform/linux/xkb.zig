...
const Parser = struct {
    ...
    fn parseKeymap(self: *Self) !void {
        ...
        if (util.debug) { util.assert(self.peek() == null); }
                ...
                .type => {
                    ...
                    while (!self.nextIs(.brace_right)) {
                        ...
                        switch (keyword_inner) {
                            .modifiers => {
                                if (util.debug) { util.assert(_type.modifiers.real == 0 and _type.modifiers.virtual == 0); }
                                try self.expect(.equal);
        ...
        while (!self.nextIs(.brace_right)) {
            switch (try self.next()) {
                .name => {
                    ...
                    if (util.debug) { util.assert(group_indices.count() == self.keymap.key_infos.items.len); }
    ...
    fn readSymbol(self: *Self) !?Symbol {
        ...
        const symbol_str = switch (symbol_token) {
            ...
            .brace_right,
            .square_left,
            .square_right => return error.UnexpectedToken,
            // keyword
    ...
    fn do(self: *Self) !?Token {
        ...
        if (std.ascii.isAlphabetic(char) or char == '_') {
            ...
            if (std.mem.eql(u8, ident, "Shift")) {
                ...
                std.mem.eql(u8, ident, "mod_map")) {  // handle aliases
                return .modifier_map;
